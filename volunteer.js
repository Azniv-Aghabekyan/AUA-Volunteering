...
function checkFunction(){
    ...
    var otherSkills = document.getElementById("other-skills").value;
    volunteeringInfo.otherSkills = otherSkills;

    var volunteers = JSON.parse(window.localStorage.getItem("volunteeringInfo"));
    if (!volunteers) {
        volunteers = []      
    }
    ...
    if (formValid == true) {
        volunteers.push(volunteeringInfo);
        window.localStorage.setItem("volunteeringInfo", JSON.stringify(volunteers));
...
function loadExistingVolunteers() {
	volunteers = JSON.parse(window.localStorage.getItem("volunteeringInfo"));
